...
- [ ] Require a checkpoint only when a mandatory human-decision pause follows persistent current-task mutations.
- [ ] Validate affected artifacts and stage only current-task paths or hunks before committing with the plan or request id and decision gate in the message.
- [ ] Skip commits for pre-mutation and no-change pauses, and forbid pre-existing, user-owned, unrelated, secret-bearing, or known-invalid content.
- [ ] Report any unavoidable dirty state and its exact paths when a safe checkpoint cannot be formed instead of creating a misleading commit.
- [ ] Keep checkpoint commits separate from final delivery and prohibit rewriting a checkpoint after another branch or worktree may rely on it.
- [ ] Synchronize the concise managed guardrail and verify both positive and negative behavior boundaries without duplicating the full policy in route skills.
...
- [ ] Confirm the related plan closure has accurate dependencies and one canonical checkpoint-policy owner.
- [ ] Obtain current package, behavior, guardrail, repository, plan-index, diff, and status evidence against the stabilized implementation.
- [ ] Complete the plan, mark F020 done, regenerate the index, and create an F020-qualified final delivery commit without rewriting the approval checkpoint.
