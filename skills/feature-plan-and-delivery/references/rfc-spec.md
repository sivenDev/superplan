...
1. 存在具体、未解决的设计决策；
2. 存在会实质改变方案的多个可行选项，或一个难以逆转的选择；
3. 错误选择会改变验收，或形成难以逆转的公共契约、迁移、安全、并发、数据完整性、发布或回滚影响；
4. 无法通过一次澄清、保守默认或普通开发计划安全解决。
...
类别或关键词命中、可逆的内部实现选择、任务/文件/模块数量、代码不熟悉和一般不确定性都不触发 RFC。边界情况先提出一个简短澄清问题。人类明确拒绝时走直接 feature 流程；若仍存在重大风险，说明风险并重新请求确认，不得静默覆盖。
