...
| RFC document or RFC-backed feature plan | Validate the human registry and plan catalog; inspect exact RFC id/path/source/status/version, required sections, selected language, and plan reference. | Require an approved matching RFC before plan creation or feature completion, then refresh/check the plan index. Metadata-only RFC wording fixes do not require code regression. |
