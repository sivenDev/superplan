...
| RFC document or RFC-backed feature plan | Validate the human registry and plan catalog; inspect the selected flat/directory layout, exact RFC id/feature/path/source/status/version, required sections, selected language, and direct plan reference. | Require every matching RFC to be approved before plan creation or feature completion, then refresh/check the plan index. Metadata-only RFC wording fixes do not require code regression. |
