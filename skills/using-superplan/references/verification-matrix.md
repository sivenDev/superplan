...
| --- | --- | --- |
| Skill or reference metadata/instructions | Run `quick_validate.py` for each affected skill folder; inspect trigger boundaries and reference ownership against the changed behavior. | Re-run structural validation for affected skills after final edits. When shared references change cross-route behavior, validate all bundled route skills and execute the applicable scenarios from `workflow-behavior-tests.md`. |
| Bundled Python scripts | Run the directly related unittest module and a CLI help/smoke path when its public arguments change. | Run `python3 -m unittest discover -s skills/using-superplan/scripts/tests` once after script behavior stabilizes. |
