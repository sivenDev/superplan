...
- Feature and bugfix plan ids encode their source human entry (e.g. `F001`, `F001@feature-x`, or split ids such as `F001-01` and `F001@feature-x-01`); the index derives the source from the id and verifies it exists, so each plan traces back to a real entry.
